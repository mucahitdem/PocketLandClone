    ...
    public class PolyverseSkiesGenerator : StyledMonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (generateCubemap)
            {
                ...
                if (cubemapType == CubemapType.CloudsCubemap) path = "Assets/Generated Clouds Cubemap.png";

                var cubemap = new Cubemap((int) cubemapSize, GraphicsFormat.R32G32B32A32_SFloat, 0);

                cam.RenderToCubemap(cubemap);
                cubemap.Apply();

                var img = new Texture2D((int) cubemapSize * 6, (int) cubemapSize, TextureFormat.RGBAFloat, false);

                for (var i = 0; i < (int) cubemapSize; i++)
                for (var j = 0; j < (int) cubemapSize; j++)
                {
